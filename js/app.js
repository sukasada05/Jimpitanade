(function() {
  ...
  // ============ TAB NAVIGATION ============
  $$('.tabs__btn').forEach(b => b.addEventListener('click', function() {
    $$('.tabs__btn').forEach(x => x.classList.remove('tabs__btn--active'));
    this.classList.add('tabs__btn--active');
    $$('.panel').forEach(p => p.classList.remove('panel--active'));
    $(`#${this.dataset.tab}`).classList.add('panel--active');
    refresh();
  }));
  ...
  function fDateTime(d) { return fDateISO(d) + ' ' + d.getHours().toString().padStart(2, '0') + ':' + d.getMinutes().toString().padStart(2, '0'); }
  function kodeTanggal(d) { const t = new Date(d); return t.getFullYear() + ('0' + (t.getMonth() + 1)).slice(-2) + ('0' + t.getDate()).slice(-2) + '_' + ('0' + t.getHours()).slice(-2) + ('0' + t.getMinutes()).slice(-2); }
  function toast(msg) { const t = $('#toast'); t.textContent = msg; t.classList.add('toast--show'); clearTimeout(t._t); t._t = setTimeout(() => t.classList.remove('toast--show'), 2500); }
  ...
  function refresh() {
        ...
        el.innerHTML += `
          <div class="edit-item">
            <div class="edit-item__info">
              <div class="edit-item__date">${fDateTime(t.tanggal)}</div>
              <div class="edit-item__nominal">${fRp(t.nominal)} <span class="text-sm">(${t.hari} hari)</span></div>
            </div>
            <div class="edit-item__actions">
              <button class="edit-item__btn edit-item__btn--edit" data-index="${i}">Edit</button>
              <button class="edit-item__btn edit-item__btn--hapus" data-index="${i}">Hapus</button>
            </div>
          </div>`;
      });
    }
    ...
    if (transaksi.length === 0) {
      ...
      let r = 0;
      transaksi.forEach(t => { r += t.nominal; tb.innerHTML += `<tr><td>${fDate(t.tanggal)}</td><td>${fRp(t.nominal)}</td><td>${t.hari} hr</td><td style="font-weight:600;">${fRp(r)}</td></tr>`; });
    }
  }
  ...
  function bangunStruk(trx) {
    ...
    let h = '';
    h += `<div class="struk__center struk__judul">-JIMPITAN Mangsatria-</div>`;
    h += `<div class="struk__center struk__sub">${desa}</div>`;
    h += `<div class="struk__garis--tebal"></div>`;
    h += `<div class="struk__row"><span>Nasabah</span><span>: ${nama}</span></div>`;
    h += `<div class="struk__row"><span>Target/hari</span><span>: ${fRp(target)}/hari</span></div>`;
    h += `<div class="struk__row"><span>Tgl setor</span><span>: ${fDate(tglMulai)} s.d. ${fDate(tglAkhir)}</span></div>`;
    h += `<div class="struk__row"><span></span><span class="struk__kanan">(${jml} hari)</span></div>`;
    h += `<div class="struk__garis--tebal"></div>`;
    for (let i = 0; i < jml; i++) { const t = new Date(tglMulai); t.setDate(t.getDate() + i); h += `<div class="struk__row"><span>${fDate(t)}</span><span class="struk__kanan">${fRp(target)}</span></div>`; }
    h += `<div class="struk__garis"></div>`;
    h += `<div class="struk__row struk__total"><span>Jumlah Setor</span><span class="struk__kanan">${fRp(trx.nominal)}</span></div>`;
    h += `<div class="struk__garis"></div>`;
    h += `<div class="struk__center">Terima kasih</div><div class="struk__center">Om Swastiastu 🙏</div>`;
    return h;
  }
  ...
  async function saveDanShare(html, trx) {
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: 'Struk Jimpitan', text: 'Struk Jimpitan - ' + nama });
      } else {
        window.open('https://wa.me/?text=' + encodeURIComponent('*JIMPITAN Mangsatria*\n' + desa + '\nNasabah: ' + nama + '\nSetor: ' + fRp(trx.nominal) + '\n🙏'), '_blank');
      }
  ...
  function setor(nominal) {
    ...
    transaksi.push(trx); hitungUlang(); refresh();
  ...
  $('#btnMulai').addEventListener('click', () => {
    ...
    refresh(); document.querySelector('[data-tab="setor"]').click();
  });
  $('#editList').addEventListener('click', e => {
    const i = parseInt(e.target.dataset.index); if (isNaN(i)) return;
    if (e.target.classList.contains('edit-item__btn--edit')) {
      $('#editIndex').value = i; $('#editNominal').value = transaksi[i].nominal;
      $('#modalEdit').classList.add('modal-overlay--show'); setTimeout(() => $('#editNominal').focus(), 100);
    }
    if (e.target.classList.contains('edit-item__btn--hapus')) {
      $('#hapusIndex').value = i; $('#hapusInfo').textContent = 'Hapus ' + fRp(transaksi[i].nominal) + ' (' + transaksi[i].hari + ' hari)?';
      $('#modalHapus').classList.add('modal-overlay--show');
    }
  });
  $('#btnBatalEdit').addEventListener('click', () => $('#modalEdit').classList.remove('modal-overlay--show'));
  $('#btnSimpanEdit').addEventListener('click', () => {
    ...
    transaksi[i].nominal = b; transaksi[i].hari = Math.floor(b / target); hitungUlang(); refresh();
    $('#modalEdit').classList.remove('modal-overlay--show');
  });
  $('#editNominal').addEventListener('keypress', e => { if (e.key === 'Enter') $('#btnSimpanEdit').click(); });
  $('#btnBatalHapus').addEventListener('click', () => $('#modalHapus').classList.remove('modal-overlay--show'));
  $('#btnKonfirmasiHapus').addEventListener('click', () => {
    transaksi.splice(parseInt($('#hapusIndex').value), 1); hitungUlang(); refresh();
    $('#modalHapus').classList.remove('modal-overlay--show');
  });
  document.querySelectorAll('.modal-overlay').forEach(o => o.addEventListener('click', function(e) { if (e.target === this) this.classList.remove('modal-overlay--show'); }));
  $('#btnReset').addEventListener('click', () => {
    if (confirm('Hapus semua data?')) {
      transaksi = []; totalSetor = 0; totalHari = 0; aktif = false;
      $('#targetHarian').value = 2000; $('#namaNasabah').value = 'Ni Luh Sari'; $('#desa').value = 'Desa Padang Bulia';
      refresh(); document.querySelector('[data-tab="setup"]').click();
    }
  });
